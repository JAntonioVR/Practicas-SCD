...
{
    while ( consumidos <= num_items-nConsumidores ){
      int dato ;
      sem_wait(ocupadas);
      mtx.lock();
      if(LIFO){
...
int main(int argc, char ** argv)
{
   ...
   if(LIFO)
    ...
    cout << "Introduce el número de productores: ";
    cin >> nProductores;
    cout << "Introduce el número de consumidores: ";
    cin >> nConsumidores;
    

   thread hebrasP[nProductores], hebrasC[nConsumidores];

   for (size_t i = 0; i < nProductores; i++)
       hebrasP[i] = thread( funcion_hebra_productora );

   for (size_t i = 0; i < nConsumidores; i++)
   {
       hebrasC[i] = thread( funcion_hebra_consumidora );
   }
