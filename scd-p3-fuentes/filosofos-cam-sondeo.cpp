...
void funcion_camarero(){
    ...
    while (true)
    {
        ...
        while (hay_mensaje==0) 
        {
            MPI_Iprobe(id_filosofo, etiq_aceptable, MPI_COMM_WORLD, &hay_mensaje, &estado);
            if(!hay_mensaje){
                id_filosofo=(id_filosofo+2)%(num_filosofos*2);
            }
        }
        
        MPI_Recv ( &valor, 1, MPI_INT, id_filosofo , etiq_aceptable, MPI_COMM_WORLD,&estado );
