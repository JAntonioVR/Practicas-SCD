...
void funcion_camarero(){
    ...
    while (true)
    {
        if(s>=num_filosofos-1){
            ...
            emisor_aceptable = MPI_ANY_SOURCE;
        }
        else{
            //Espero a que el que tiene el turno quiera sentarse
            MPI_Probe(turno, ETIQ_SENTARSE, MPI_COMM_WORLD, &estado); 

            MPI_Iprobe(MPI_ANY_SOURCE, ETIQ_LEVANTARSE, MPI_COMM_WORLD, &flag, &estado );
            if(flag>0){      // Alguien quiere levantarse
                etiq_aceptable = ETIQ_LEVANTARSE;
                emisor_aceptable = estado.MPI_SOURCE;
            }
            else{
                etiq_aceptable = ETIQ_SENTARSE;
                emisor_aceptable = turno;
            }

        }
        MPI_Recv ( &valor, 1, MPI_INT, emisor_aceptable , etiq_aceptable, MPI_COMM_WORLD,&estado );
