...
#define ETIQ_PRODUCTOR  2
#define ETIQ_CONSUMIDOR_PARES 0
#define ETIQ_CONSUMIDOR_IMPARES 1
...
void funcion_consumidor(int num_consumidor)
{
    int         peticion,
                valor_rec = 1 ,
                tag = (num_consumidor % 2) ? ETIQ_CONSUMIDOR_IMPARES : ETIQ_CONSUMIDOR_PARES;
    MPI_Status  estado ;
...
void funcion_buffer()
{
    ...
    for( unsigned int i=0 ; i < num_items*2 ; i++ )
    {
        ...
        int etiq_cons = (buffer[primera_ocupada]%2) ? ETIQ_CONSUMIDOR_IMPARES :
                                                      ETIQ_CONSUMIDOR_PARES;

        if ( num_celdas_ocupadas == 0 )               // si buffer vacío
            etiq_emisor_aceptabe = ETIQ_PRODUCTOR ;   // $~~~$ solo prod.
        else if ( num_celdas_ocupadas == tam_vector ){ // si buffer lleno
            etiq_emisor_aceptabe = etiq_cons;
        }
        else{                                          // si no vacío ni lleno
            do{
                MPI_Iprobe(MPI_ANY_SOURCE, ETIQ_PRODUCTOR, MPI_COMM_WORLD, &flag_prod, &estado);
                MPI_Iprobe(MPI_ANY_SOURCE, etiq_cons, MPI_COMM_WORLD, &flag_cons, &estado);
            }while (flag_prod == 0 && flag_cons == 0);

            if(flag_prod>0) etiq_emisor_aceptabe = ETIQ_PRODUCTOR;
            else etiq_emisor_aceptabe = etiq_cons;
        }
        ...
        switch( estado.MPI_TAG ) // leer emisor del mensaje en metadatos
        {
            ...
            case ETIQ_CONSUMIDOR_PARES: // si ha sido el consumidor: extraer y enviarle
            case ETIQ_CONSUMIDOR_IMPARES:
                valor = buffer[primera_ocupada] ;
                primera_ocupada = (primera_ocupada+1) % tam_vector ;
                num_celdas_ocupadas-- ;
                cout << "Buffer va a enviar valor " << valor << endl ;
                MPI_Ssend( &valor, 1, MPI_INT, estado.MPI_SOURCE, estado.MPI_TAG, MPI_COMM_WORLD);
                break;
        }
    }
}
